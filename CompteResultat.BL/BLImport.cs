...
namespace CompteResultat.BL
{
    public class BLImport
    {
        ...
        public string NewPrestCSV { get; set; }
        public string NewCotCSV { get; set; }
        public string NewDemoCSV { get; set; }
        ...
        public BLImport(string userName, string newPrestCSV, string newCotCSV, string newDemoCSV, string newOtherFieldsCSV,
            string newCotPrevCSV, string newSinistrePrevCSV, string newDecompPrevCSV,
            ...
            bool forceCompanySubsid, bool updateGroupes, bool updateExperience, bool updateCad)
        {
            ...
            UserName = userName;
            NewPrestCSV = newPrestCSV;
            NewCotCSV = newCotCSV;
            NewDemoCSV = newDemoCSV;
        ...
        public void DoImport()
        {
            try
            {
                ...
                if (File.Exists(NewDemoCSV) && !UploadPathDemo.EndsWith(UserName + "_"))
                    BulkInsert.DoBulkInsert(NewDemoCSV, C.eDBImportTables.Demography.ToString());
                if (File.Exists(NewPrestCSV) && !UploadPathPrest.EndsWith(UserName + "_"))
                    BulkInsert.DoBulkInsert(NewPrestCSV, C.eDBImportTables.PrestSante.ToString());
                if (File.Exists(NewCotCSV) && !UploadPathCot.EndsWith(UserName + "_"))
                    BulkInsert.DoBulkInsert(NewCotCSV, C.eDBImportTables.CotisatSante.ToString());
        ...
        public void DoImportOLD()
        {
            try
            {
                ...
                // ***** Bulk Insert converted CSV files into TEMP Tables: Prest + Cot + Demo + OtherFields

                //### do we really need the following 3 temp tables: Prest, Cotisat & Demo ???

                //if (File.Exists(newPrestCSV))
                //    BulkInsert.DoBulkInsert(newPrestCSV, C.eDBTempTables._TempPrestation.ToString());
                //if (File.Exists(newCotCSV))
                //    BulkInsert.DoBulkInsert(newCotCSV, C.eDBTempTables._TempCotisation.ToString());
                //if (File.Exists(newDemoCSV))
                //    BulkInsert.DoBulkInsert(newDemoCSV, C.eDBTempTables._TempDemography.ToString());

                if (File.Exists(NewOtherFieldsCSV))
                    BulkInsert.DoBulkInsert(NewOtherFieldsCSV, C.eDBTempTables._TempOtherFields.ToString());


                // ***** Eventually Update additional Tables: Assur, Company, Contract 
                ...
                foreach (string contr in contractsToBeAdded)
                {
                    ...
                    //with the AssureurName, get the Assureur Entity from the Assureur Table
                    //Assureur ass = Assureur.GetAssByName(assName);
                    //if (ass == null)
                    //    throw new Exception("No insurance company ID was found in the 'Assureur' table for the following insurance company: " + assName);

                    int contrId = Assureur.AddContractToAss(new Contract { ContractId = contr, ImportId = importId }, assId);
                }

                //Verify Parent Comp
                List<string> existingParComp = Company.GetUniqueParentCompanies();
                List<string> newParComp = C_TempOtherFields.GetUniqueParentCompanies();
                //List<string> existingParComp2 = Company.GetUniqueParentCompanyContractIdPairs();
                //List<string> newParComp = C_TempOtherFields.GetUniqueParentCompanyContractIdPairs();
                List<string> compsToBeAdded = newParComp.Where(i => !existingParComp.Contains(i)).ToList();

                //### handle case where 1 company has 2 contracts

                foreach (string comp in compsToBeAdded)
                {

                    //get all contracts for this comp => loop


                    //first we add the company
                    int compId = Company.Insert(new Company { ImportId = importId, Name = comp });

                    //we search the associated ContrId in the Temp Table
                    string contractName = ""; //### C_TempOtherFields.GetContractNameForCompany(comp);
                    if (contractName == C.cINVALIDSTRING)
                        throw new Exception("No contract was found in the '_TempOtherFields' table for the following company: " + comp);

                    //with the ContractName, get the ContractId from the Contract Table
                    int contrId = Contract.GetContrIdForContrName(contractName);
                    if (contrId == C.cINVALIDID)
                        throw new Exception("No ContractID was found in the 'Contract' table for the following Contract: " + contractName);

                    Company.AddContrToCompany(new Contract { ContractId = contractName, ImportId = importId }, compId);

                    //### OLD

                    //string contractName = Regex.Split(compContr, C.cSTRINGSEP)[0];
                    //string comp = Regex.Split(compContr, C.cSTRINGSEP)[1];

                    //get the ContractName for the given Company from TempTable
                    //string contractName = contrName; // C_TempOtherFields.GetContractNameForCompany(comp);
                    //if (contractName == C.cINVALIDSTRING)
                    //    throw new Exception("No contract was found in the '_TempOtherFields' table for the following company: " + comp);
                    ...
                    //int compId = Company.Insert(new Company { ImportId = importId, ContractId = contrId, Name = comp });
                }
                ...
                List<string> existingSubsid = Company.GetUniqueSubsids();
                List<string> newSubsid = C_TempOtherFields.GetUniqueSubsids();
                //List<string> existingSubsid = Company.GetUniqueSubsidContractIdPairs();
                //List<string> newSubsid = C_TempOtherFields.GetUniqueSubsidContractIdPairs();
                List<string> subsToBeAdded = newSubsid.Where(i => !existingSubsid.Contains(i)).ToList();
                ...
                foreach (string sub in subsToBeAdded)
                {
                    ...
                    //we search the associated ContrId in the Temp Table
                    string contractName = ""; //### C_TempOtherFields.GetContractNameForSubsid(sub);
                    if (contractName == C.cINVALIDSTRING)
                        throw new Exception("No contract was found in the '_TempOtherFields' table for the following company: " + sub);
                    ...
                    //string contrName = Regex.Split(subContr, C.cSTRINGSEP)[0];
                    //string subsid = Regex.Split(subContr, C.cSTRINGSEP)[1];

                    ////get the ContractName for the given Company from TempTable
                    //string contractName = contrName; // C_TempOtherFields.GetContractNameForSubsid(subs);
                    //if (contractName == C.cINVALIDSTRING)
                    //    throw new Exception("No contract was found in the '_TempOtherFields' table for the following subsidiary: " + subsid);
                    ...
                    ////get the name of the associated Parent Company
                    //string parCompName = C_TempOtherFields.GetParentCompanyNameForSubsid(subsid);
                    //if (parCompName == C.cINVALIDSTRING)
                    //    throw new Exception("No parent company was found in the '_TempOtherFields' table for the following subsidiary: " + subsid);
                    ////get the id of the associated Parent Company
                    //int parCompId = Company.GetCompIdForParentCompName(parCompName);
                    //if (parCompId == C.cINVALIDID)
                    //    throw new Exception("No parent company ID was found in the 'Company' table for the following parent company: " + parCompName);

                    //###int compId = Company.Insert(new Company { ImportId = importId, ContractId = contrId, Name = subsid, ParentId = parCompId });
                }
